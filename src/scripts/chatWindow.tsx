...
import { groupMessages, MessageGroup, sanitizeMessage } from './util';

const createSocket = async (fkey: string, chatRoomId: number, setSocket: (ws: WebSocket) => void, setMessages: (cb: (g: MessageGroup[]) => MessageGroup[]) => void): Promise<WebSocket> => {
...
const ChatWindow = ({ chatRoomId }: { chatRoomId: number }) => {
    ...
    useEffect(
        () => {
            ...
            const socketPromise = createSocket(fkey.fkey, chatRoomId, setSocket, setMessages);
